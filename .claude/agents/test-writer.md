...
- Read `CLAUDE.md`'s `wrap()` section in full if you're touching `client.py` or
  `clients/js/src/` — it documents six specific, previously-invisible edge cases. Don't write
  tests that only exercise the naive path; each of these six needs its own case if it isn't
  already covered:
  ...
  4. `tools_used` comes from `tool_use` content blocks; `web_search_requests` comes from
     `response.usage.server_tool_use` — test them as genuinely separate paths, not one mock that
